...
(<any>window).walnut = (function(window) {
	"use strict";
	...
	*/
	function findAncestor (el: HTMLElement, cls: string) {
		let elem = el;
	...
	let launchIntoFullscreen: any = undefined,
		exitFullscreen: any = undefined;

	if (!!isFullscreenEnabled()) {

		let fullscreenEnabled = (<any>document).fullscreenEnabled || (<any>document).mozFullScreenEnabled || (<any>document).webkitFullscreenEnabled;
		let fullscreenElement = (<any>document).fullscreenElement || (<any>document).mozFullScreenElement || (<any>document).webkitFullscreenElement;
	 ...
	 */
	function resizeEvent(callback: (...args: any[]) => void, action: string = undefined) {
		if(action === "remove") {
	...
	let walnut = (function() {
		 ...
		 */
		let initEvents = utils.once(function() {
		...
		function openViewer(e: any) {
			...
			let src;
			let style = this.currentStyle || window.getComputedStyle(this, null);
		...
		function fixListWidth() {
			let elItem: any = document.getElementsByClassName("walnut__item")[0],
				listItem: number = elItem.offsetWidth,
				elList: any = document.getElementsByClassName("walnut__list")[0];
